    ...
    \"resources\":
        {
            ...
            \"uefi\":               \"$(realpath ${VTL0_KERNEL})\"
        }
}"

echo ${RESOURCES} > openhcl-x64-snp-dev-direct-resources.json

cat <<EOF > openhcl-x64-snp-dev-direct.json
{
    "guest_arch": "x64",
    "guest_configs": [
        {
            ...
            "image": {
                "openhcl": {
                    "command_line": "OPENHCL_FORCE_LOAD_VTL0_IMAGE=linux OPENHCL_BOOT_LOG=com3 OPENHCL_REPORT_VTL0_STARTED=1",
...
# Combine everything into a single IGVM image

cargo run -p igvmfilegen -- manifest -m ./openhcl-x64-snp-dev-direct.json -r openhcl-x64-snp-dev-direct-resources.json -o openhcl-x64-snp-dev-direct.igvm
